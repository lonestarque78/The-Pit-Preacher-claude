...
export async function generateInsights(
  ...
  if (!cook) {
    return { patternInsights: [], pitInsights: [], nextTimeRecommendations: [] };
  }

  const pitType: string = cook.smoker_type?.toLowerCase() ?? "unknown";
  const meatType: string = cook.label?.toLowerCase() ?? "";

  // ── Load past cooks for pattern detection ───────────────────────────────
  const { data: pastCooks } = await supabase
    .from("cooks")
    .select("id, smoker_type, label, created_at, completed_at, eat_time, plan")
    .eq("user_id", userId)
    .eq("status", "completed")
    .neq("id", cookId)
  ...
  if (outcome?.finish_time_actual && cook.eat_time) {
    ...
    if (diff > 30) {
      patternInsights.push(
        `This cook finished ${minutesToHours(diff)} after your planned eat time. Build a longer buffer into your next timeline.`
      );
    } else if (diff < -60) {
      patternInsights.push(
        `This cook finished ${minutesToHours(Math.abs(diff))} early. Your buffer worked — the cooler rest improved the final product.`
      );
    }
  }
  ...
  if (outcome?.stall_time_minutes) {
    if (outcome.stall_time_minutes > 180) {
      patternInsights.push(
        `Your stall ran ${minutesToHours(outcome.stall_time_minutes)}. That's a long one — next time consider wrapping at the two-hour mark to reclaim time.`
      );
    } else if (outcome.stall_time_minutes < 45) {
      patternInsights.push(
        `Short stall of ${minutesToHours(outcome.stall_time_minutes)}. Your pit humidity is working — keep the environment consistent.`
      );
    }
  }

  // Bark quality vs moisture tradeoff
  if (outcome?.bark_quality && outcome?.moisture_level) {
    if (outcome.bark_quality <= 2 && outcome.moisture_level >= 4) {
      patternInsights.push(
        "High moisture but weak bark — you likely wrapped too early or too tight. Let the bark set before sealing it in."
      );
    }
    if (outcome.bark_quality >= 4 && outcome.moisture_level <= 2) {
      patternInsights.push(
        "Strong bark but dry finish — great crust development, but the meat needs more rest time or an earlier wrap to hold moisture."
      );
    }
  }

  // Past cook patterns — repeated bark issues
  const lowBarkCooks = pastOutcomes.filter((o: any) => o.bark_quality && o.bark_quality <= 2);
  if (lowBarkCooks.length >= 2) {
    patternInsights.push(
      `Bark has been weak across ${lowBarkCooks.length} recent cooks. Check your wrap timing and make sure you're running dry heat in the final hour.`
    );
  }

  // Past cook patterns — repeated moisture loss
  const dryFinishes = pastOutcomes.filter((o: any) => o.moisture_level && o.moisture_level <= 2);
  if (dryFinishes.length >= 2) {
    patternInsights.push(
      `Dry finishes are showing up across multiple cooks. Wrap earlier, rest longer, or reduce your cook temp by 15–20°F.`
    );
  }
  ...
  if (outcome?.smoke_profile && outcome.smoke_profile <= 2) {
    patternInsights.push(
      "Light smoke profile this cook. Add a smoke tube in the first two hours or switch to a stronger wood like hickory or oak."
    );
  }
  if (outcome?.smoke_profile && outcome.smoke_profile >= 5 && outcome.flavor_balance && outcome.flavor_balance <= 3) {
    patternInsights.push(
      "Heavy smoke overpowered the flavor balance. Pull back to fruit wood or reduce your smoke exposure window."
    );
  }

  // Event-based patterns — pit spikes
  ...
  if (spikeEvents.length >= 2) {
    patternInsights.push(
      `${spikeEvents.length} temp spikes logged. Your fire is running reactive — make smaller vent adjustments and give the pit five minutes before touching anything again.`
    );
  }

  // Event-based patterns — lid openings
  ...
  if (lidEvents.length >= 4) {
    patternInsights.push(
      `You opened the lid ${lidEvents.length} times. Every opening drops pit temp and slows the stall. Trust the process — check less.`
    );
  }
  ...
  if (pitType.includes("offset")) {
    ...
    if (outcome?.bark_quality && outcome.bark_quality >= 4) {
      pitInsights.push("Strong bark on an offset — that dry convective heat is doing its job. Keep the fire active.");
    }
  }
  ...
  if (pitType.includes("kamado")) {
    ...
    if (outcome?.stall_time_minutes && outcome.stall_time_minutes < 60) {
      pitInsights.push("Short stall is expected on a kamado — the ceramic retains heat and keeps evaporative cooling in check.");
    }
  }
  ...
  if (pitType.includes("kettle")) {
    ...
    if (outcome?.pit_temp_high && outcome.pit_temp_low && (outcome.pit_temp_high - outcome.pit_temp_low) > 50) {
      pitInsights.push(`Wide temp range of ${outcome.pit_temp_low}–${outcome.pit_temp_high}°F on a kettle means fuel management needs attention. Add smaller amounts more frequently.`);
    }
  }

  if (pitType.includes("drum") || pitType.includes("uds")) {
    pitInsights.push("Drums run hotter than expected. If your target is 250°F, start with vents barely cracked and work up from there.");
    ...
    if (outcome?.moisture_level && outcome.moisture_level >= 4) {
      pitInsights.push("Great moisture retention — the drum environment is working. Keep your vents consistent and don't over-adjust.");
    }
  }
  ...
  if (pitType.includes("electric")) {
    pitInsights.push("Add wood chips in the first two hours only — the electric environment amplifies smoke fast and late addition turns bitter.");
    ...
    if (outcome?.bark_quality && outcome.bark_quality <= 2) {
      pitInsights.push("Weak bark on electric is expected without intervention. A cast iron diffuser plate or finishing on a hot grill for 10 minutes helps.");
    }
  }

  // Pit temp range insight
  if (outcome?.pit_temp_low && outcome?.pit_temp_high) {
    const range = outcome.pit_temp_high - outcome.pit_temp_low;
    if (range > 75) {
      pitInsights.push(
        `Your pit swung ${range}°F during this cook. That much variance stresses the meat and makes timing unpredictable. Focus on vent consistency.`
      );
    } else if (range <= 25) {
      pitInsights.push(
        `Tight temp range of ${range}°F — excellent pit management. That consistency shows in the final product.`
      );
    }
  }
  ...
  if (notes.length > 0) {
    notes.slice(0, 3).forEach(note => {
      nextTimeRecommendations.push(note);
    });
  }

  // From outcome ratings
  ...
  // Planned vs actual time gap
  if (outcome?.start_time_actual && outcome?.finish_time_actual) {
    const actualDuration = diffMinutes(outcome.start_time_actual, outcome.finish_time_actual);
    if (actualDuration > 0 && cook.eat_time) {
      const plannedDuration = diffMinutes(outcome.start_time_actual, cook.eat_time);
      const variance = actualDuration - plannedDuration;
      if (variance > 60) {
        nextTimeRecommendations.push(
          `This cook ran ${minutesToHours(variance)} longer than planned. Start your fire ${minutesToHours(Math.min(variance, 120))} earlier next time and build in a two-hour cooler hold.`
        );
      }
    }
  }
  ...
  // Fire issues noted
  if (outcome?.fire_issues && outcome.fire_issues.trim().length > 0) {
    nextTimeRecommendations.push(
      `Fire issue noted: "${outcome.fire_issues.slice(0, 80)}${outcome.fire_issues.length > 80 ? "..." : ""}". Address this before the next cook.`
    );
  }

  // Weather impact noted
  if (outcome?.weather_impact && outcome.weather_impact.trim().length > 0) {
    nextTimeRecommendations.push(
      `Weather was a factor: "${outcome.weather_impact.slice(0, 60)}${outcome.weather_impact.length > 60 ? "..." : ""}". Add extra time to your estimate when conditions are similar.`
    );
  }

  // Cap all arrays at 5 items
  return {
