...
export default function LiveModePage({ params, onNavigate }: { params: Promise<{ id: string }>; onNavigate?: (tab: string) => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <Link href="/dashboard?fix=1" style={{
          flexShrink: 0,
          display: "inline-block",
      ...
      <div style={{
        ...
        {/* Suggested prompts */}
        {!isThinking && suggestedPrompts.length > 0 && !activeTopic && !readOnly && (
          <div style={{
            display: "flex",
            gap: "var(--space-2)",
            padding: "var(--space-2) var(--space-4)",
        ...
        {cook.status === "in_progress" && !inputDisabled && (
          <div style={{ flexShrink: 0, textAlign: "center", padding: "var(--space-1) var(--space-4)" }}>
            ...
            <button onClick={() => onNavigate?.("summary")} style={{ fontFamily: "var(--font-ui)", fontSize: "0.8rem", color: "#C9973A", textDecoration: "none", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
              Complete This Cook →
            </button>
          </div>
        )}
